...
Deploy OpenClaw on Render using Infrastructure as Code. The repo-root `render.yaml` Blueprint defines the supported Render service, disk, and environment contract so you can deploy with a single click and version your infrastructure alongside your code.
...
Render Blueprints are YAML files that define your infrastructure. The repo-root `render.yaml` in this repository configures everything needed to run OpenClaw:
...
The root blueprint is the canonical deployment source for this repo. If you keep any deployment-specific recovery templates in your fork, treat them as manual recovery artifacts only, not as additional active Blueprints.
...
For Docker services, Render exposes service environment variables at both build time and runtime. That means Docker build args such as `OPENCLAW_DOCKER_APT_PACKAGES` and `OPENCLAW_EXTENSIONS` can be driven from the same service environment when you intentionally need them. See [Docker on Render](https://render.com/docs/docker) and [Default Environment Variables](https://render.com/docs/environment-variables).
...
- Persisted config drift — inspect `$OPENCLAW_CONFIG_PATH` on the mounted disk first, especially after older manual JSON5/template-based deploys
